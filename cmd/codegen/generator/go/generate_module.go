...
func (g *GoGenerator) syncModReplaceAndTidy(mod *modfile.File, genSt *generator.GeneratedState, modPath string) error {
	...
	// Otherwise, we install the given dagger.io/dagger package version, unless
	// LibVersion is empty (offline mode, gated by DAGGER_GO_SDK_OFFLINE on the
	// engine). In that case, drop any pre-existing require so `go mod tidy`
	// doesn't try to resolve dagger.io/dagger from the proxy — the generated
	// user module imports github.com/dagger/querybuilder directly and doesn't
	// need dagger.io/dagger at runtime.
