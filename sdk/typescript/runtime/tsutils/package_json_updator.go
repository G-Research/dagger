...
func UpdatePackageJSONForModule(packageJSON string) (string, error) {
	...
	// Remove "@github.com/G-Research/dagger" from dependencies if it's set
	...
	if err != nil {
		return "", fmt.Errorf("failed to delete @github.com/G-Research/dagger dependency: %w", err)
	}

	// Remove "@github.com/G-Research/dagger" from devDependencies if it's set
	// so we smoothly transition from local to module
